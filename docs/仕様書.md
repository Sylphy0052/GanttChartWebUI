...
- 保持: 最大30日または世代10
- 保存先は Docker コンテナ内の `/app/logs` ディレクトリに出力し、`docker-compose.yml` でホストの `logs/` にマウントする。
- アップロード画像は `/app/uploads` ディレクトリに保存し、
- 同様にホストの `uploads/` にマウントする。

## 17. インフラ環境

- 利用基盤: Docker + docker-compose
- サービス構成:
  - db: PostgreSQL 15
  - backend: NestJS (Prisma利用)
  - frontend: Next.js
  - nginx: リバースプロキシ
- 環境変数は `.env` ファイルで管理
- ボリューム: `uploads/`, `logs/`, `db_data`
- 公開ポート: 8080 (nginx経由)
