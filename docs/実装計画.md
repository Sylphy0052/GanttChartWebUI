...
- Asia/Tokyo タイムゾーン
- mono-repo 構成
- 本システムは Docker 環境で構築・運用する
- ローカル開発用と本番用で docker-compose を切替可能にする

---

## 1. マイルストーン（WBS）

- M0: Docker 環境構築（compose, backend/frontend Dockerfile, nginx.conf）
- M1: 基盤整備
- M2: データ層 & API骨格
...
- ガント範囲E2E（期間外スクロール不可）
- 依存線矢印表示確認
- Docker compose で起動 (db, backend, frontend, nginx)
- Prisma migrate は backend コンテナ内で `npx prisma migrate deploy`
- ログとアップロードはホストに volume マウント
- nginx が `/api` を backend に、`/` を frontend に、`/ws` を WS Gateway にリバースプロキシ
- 環境変数を `.env` ファイルで一元管理
