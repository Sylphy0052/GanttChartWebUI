/**
 * WebSocket通知機能のテストスクリプト（ポート3010版）
 * 
 * テスト内容:
 * 1. WebSocket接続の確立
 * 2. 設定変更APIの実行
    ...
    socket.on('notification', (notification) => {
      ...
      if (notification.event === 'settings_changed') {
        ...
        if (data.message && data.timestamp && data.requiresReauth !== undefined) {
          ...
          console.log(`   再認証要求: ${data.requiresReauth}`);
        } else {
 ...
 * 設定変更API呼び出し
 */
async function callSettingsUpdateAPI(settings) {
  console.log('\n=== 設定変更API呼び出し ===');
  console.log('設定内容:', JSON.stringify(settings, null, 2));
...
async function runWebSocketNotificationTest() {
  ...
  try {
    ...
    // ステップ2: 設定変更API実行（パターン1）
    console.log('\n--- テストケース1: 土日休みOFF、固定休日追加 ---');
    ...
    // ステップ4: 設定変更（パターン2）
    console.log('\n--- テストケース2: 土日休みON、固定休日変更 ---');
    await callSettingsUpdateAPI({
      weekend_off: true,
      holiday_dates: ['2024-01-01', '2024-05-03', '2024-05-04', '2024-08-11'],
    });

    // ステップ5: 通知受信待機
    await sleep(3000);

    // ステップ6: 設定変更（パターン3 - 部分更新）
    console.log('\n--- テストケース3: 土日休みのみ変更 ---');
    await callSettingsUpdateAPI({
      weekend_off: false,
    });
    ...
    console.log('');
    console.log('🎯 実装完了事項:');
    console.log('  - 設定変更時のWebSocket通知配信');
    console.log('  - 既存セッションの再認証促進機能（準備完了）');
    console.log('  - 設定変更イベントの配信');
    console.log('  - 通知データ構造の標準化');
